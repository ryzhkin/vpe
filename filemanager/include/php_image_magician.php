...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
    ...
    #
  {
    ...
    $sx = imagesx($stamp);
    $sy = imagesy($stamp);
  /*  $stamp_out = imagecreatetruecolor($sx, $sy);
    imageAlphaBlending($stamp_out, false);
    imageSaveAlpha($stamp_out, true);
    imagecopy($stamp_out, $stamp, 0, 0, 0, 0, $sx, $sy);*/
   ...
   if (fix_strtolower(strrchr($watermarkImage, '.')) == '.png') {
    ...
    //imagecopymerge($im, $stamp, $x, $y, 0, 0, imagesx($stamp), imagesy($stamp), $opacity);
    //$this->imagecopymerge_alpha($im, $stamp, $x, $y, 0, 0, imagesx($stamp), imagesy($stamp), $opacity);
  }
  ...
  public function log($msg) {
      ...
      $today = date("d.m.Y");
      //$filename = sfConfig::get('sf_log_dir')."/{$tag}_{$today}.txt";
      $filename = "{$today}.txt";
  ...
  public function addWaterMarkText ($text) {
    /*  protected $imageResized;
      private $widthOriginal;     # Always be the original width
      private $heightOriginal;
      private $width;         # Current width (width after resize)
      private $height;*/
/*
     $font = $this->getTextFont('arial.ttf');
      $font = 'arial.ttf';
      $font_size = 55;
      $white = imagecolorallocate($this->imageResized, 255, 255, 255);
      imagettftext($this->imageResized, $font_size, 0, 20, 20, $white, $font, $text);
      ...
      $this->addWatermark('watermark.png', 'm', 100);



      // Load the stamp and the photo to apply the watermark to
     /* $stamp = $this->openImage('watermark.png'); # stamp
      $photo = $this->imageResized;               # photo
      ...
      // creating a out resource
      $photo_out = imagecreatetruecolor($photo_width, $photo_height);

      imagecopy($photo_out, $photo, 0, 0, 0, 0, $photo_width, $photo_height);

      $stamp_out = imagecreatetruecolor($stamp_width, $stamp_height);

      imagecopy($stamp_out, $stamp, 0, 0, 0, 0, $stamp_width, $stamp_height);

      imagecopymerge($photo_out, $stamp_out, 0, 0,  0, 0, $stamp_width, $stamp_height, 10);
      $this->imageResized = $photo_out;*/
...
## --------------------------------------------------------
  ...
  #       third party class to save as bmp.
    ...
    $error = '';
        ...
        if ((imagetypes() & IMG_JPG) /*|| (imagetypes() & IMG_PNG)*/) {
